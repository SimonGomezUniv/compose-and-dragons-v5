...
// startInteractiveChat starts an interactive roleplay session with the NPC in TUI mode
func startInteractiveChat(ctx context.Context, engineURL, npcAgentModelID, sheetJsonFilePath string, ragAgent *rag.Agent, compressorAgent *compressor.Agent) error {
	...
	// === INTERACTIVE NPC CHAT ===
	display.Infof("💬 Starting interactive TUI chat with NPC...")
	display.Separator()
	...
	for {
		...
		if npcAgent.GetContextSize() > 80000 {
