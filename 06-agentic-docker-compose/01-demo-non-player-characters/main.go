...
func main() {
	...
	tuiMode := flag.Bool("tui", false, "Start in TUI (Terminal UI) mode instead of web server mode")
	flag.Parse()
	...
	// === START NPC IN SELECTED MODE ===
	if *tuiMode {
		// Start in TUI (Terminal User Interface) mode
		display.Infof("🖥️  Starting in TUI mode...")
		startInteractiveChat(ctx, engineURL, npcModelID, sheetJsonFilePath, ragAgent, compressorAgent)
	} else {
		// Start in web server mode (default)
		display.Infof("🌐 Starting in web server mode...")
