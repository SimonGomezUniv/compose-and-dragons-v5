...
func initializeDefaultGame() *GameState {
	...
	playerX := 6
	if gs.hasNPCOrMonster(gs.Player.RoomId) {
		playerX = 9
	}

	// Move player to appropriate position
	gs.Player.MoveToRoom(dungeon.MoveToRoomArgs{
		Id:         gs.Player.Id,
		RoomNumber: gs.Player.RoomId,
		X:          playerX,
		Y:          2,
	})

	// Force player color to ColorBrightBlue to ensure it's always preserved
	gs.forcePlayerColor()
