...
// ansiEscape matches ANSI terminal escape sequences (colors, styles, etc.)
var ansiEscape = regexp.MustCompile(`\x1b\[[0-9;]*[mKHFJsu]|\x1b\][^\x07]*\x07|\x1b[()][AB012]`)
...
func handleGetMap(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	mapStr := stripANSI(gameState.Dungeon.GetDetailedGridCompact())
	result := map[string]interface{}{
		"map": mapStr,
	}
	return mcp.NewToolResultText(toJSON(result)), nil
